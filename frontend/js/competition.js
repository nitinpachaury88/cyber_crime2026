...
(async function () {
  ...
  // ==========================================================================
  // Overall 45-minute assessment countdown. This is a SERVER-OWNED clock:
  // the backend sets assessment_started_at / assessment_ends_at the first
  // time a team opens the assessment, and enforces expiry itself (see
  // GET /assessment/state). The frontend never starts, stores, or resets
  // the timer — it only renders a countdown from state.assessmentEndsAt,
  // corrected for any difference between this machine's clock and the
  // server's clock (via state.serverNow). This is what makes retakes,
  // refreshes, and reopening the assessment on a different device all show
  // the same, correct remaining time.
  // ==========================================================================
  function startAssessmentTimer() {
    const timerEl = document.getElementById('assessmentTimer');
    if (!timerEl || !state || !state.assessmentEndsAt) return;

    const serverNowMs = new Date(state.serverNow).getTime();
    const endsAtMs = new Date(state.assessmentEndsAt).getTime();
  ...
  load().then(startAssessmentTimer);
})();