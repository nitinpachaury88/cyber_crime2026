...
def _as_utc(dt):
    """MySQL (and SQLite) don't persist tzinfo, so a DateTime(timezone=True)
    column comes back timezone-naive after a round trip through the DB even
    though it was saved as UTC-aware. Comparing that naive value directly
    against datetime.now(timezone.utc) raises TypeError, and — just as
    importantly — serializing it as-is to JSON produces a string with no
    "Z"/"+00:00" offset, which browsers then parse as LOCAL time instead of
    UTC. Treat any naive value as already being UTC and attach the tzinfo
    back before comparing OR returning it to the client."""
...
async def get_assessment_state(db: Session = Depends(get_db), user: CurrentUser = Depends(get_current_user)):
    ...
    return {
        ...
        #
        # IMPORTANT: both timestamps are passed through _as_utc() before
        # being returned. MySQL strips tzinfo from DateTime(timezone=True)
        # columns on the way back out, so without this, FastAPI/pydantic
        # would serialize a naive datetime (no "Z"/"+00:00" suffix). The
        # browser's `new Date(...)` then parses that string as LOCAL time
        # instead of UTC, which — for anyone ahead of UTC (e.g. IST,
        # UTC+5:30) — makes the computed deadline look like it's already
        # in the past, so the countdown shows 00:00 immediately.
        "serverNow": datetime.now(timezone.utc),
        "assessmentStartedAt": _as_utc(team.assessment_started_at),
        "assessmentEndsAt": _as_utc(team.assessment_ends_at),
    }
...
async def retake_assessment(db: Session = Depends(get_db), user: CurrentUser = Depends(get_current_user)):
    ...
    # Also reset the 45-minute clock — without this, the old (already
    # expired) assessment_ends_at survives the retake, and the very next
    # /state call sees "time's up" and auto-finalizes again immediately.
    if team:
        team.assessment_started_at = None
        team.assessment_ends_at = None
